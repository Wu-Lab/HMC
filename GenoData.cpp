...
GenoData::GenoData(int num, int len)
: m_genotype_num(0),
  m_genotype_len(0),
  m_unphased_num(0)
{
...
int GenoData::getAlleleIndex(int locus, Allele a) const
{
	int i;
	bool found;
	found = false;
...
void GenoData::setGenotypeNum(int num)
{
	int i;
...
void GenoData::setGenotypeLen(int len)
{
	int i;
...
void GenoData::checkAlleleSymbol()
{
	int i, j, k, l;
	for (i=0; i<m_genotype_len; ++i) {
...
int GenoData::max_allele_num() const
{
	int i, m;
	m = 0;
	for (i=0; i<m_genotype_len; ++i) {
